...
const waveformContext = solidJs.createContext({
  zoom: 0,
  duration: 0,
  position: 0,
  updatePosition: () => {},
...
const useWaveformContext = () => solidJs.useContext(waveformContext);
...
var _tmpl$$4 = /*#__PURE__*/web.template(`<div class=Waveform-Region style=display:flex;justify-content:space-between;position:absolute;top:0;height:100%;opacity:0.7>`),
  _tmpl$2$1 = /*#__PURE__*/web.template(`<div style="border-left:5px rgba(0, 0, 0, 0) solid;height:100%">`);
const regionDragHandles = ["LEFT", "MIDDLE", "RIGHT"];
const Region = props => {
  const viewPort = useWaveformViewPortScaler();
  const virtualDimensions = solidJs.createMemo(() => viewPort.getVirtualDimensions(props.region.start, props.region.end - props.region.start));
  ...
  return web.createComponent(solidJs.Show, {
    ...
    get children() {
      ...
      web.insert(_el$, web.createComponent(solidJs.For, {
        ...
        children: dragHandle => web.createComponent(RegionDragHandle, {
      ...
      web.effect(_p$ => {
        ...
        _v$ !== _p$.e && web.setStyleProperty(_el$, "width", _p$.e = _v$);
        _v$2 !== _p$.t && web.setStyleProperty(_el$, "left", _p$.t = _v$2);
        _v$3 !== _p$.a && web.setStyleProperty(_el$, "background-color", _p$.a = _v$3);
        return _p$;
      }, {
        e: undefined,
        t: undefined,
...
var _tmpl$$3 = /*#__PURE__*/web.template(`<div class=Waveform-Spinner style=display:flex;position:absolute;left:0;top:0;width:100%;height:100%;justify-content:center;align-items:center;background:rgba(0,0,0,0.1)>`),
...
const Waveform = allProps => {
  ...
  const [props, divProps] = solidJs.splitProps(propsWithDefauls, ["strokeStyle", "buffer", "position", "zoom", "scale", "logScale", "mode", "lineWidth", "onScaleChange", "onPositionChange", "onZoomChange", "children"]);
  let scrollbarDivRef;
  let canvasRef;
  let context;
  const rawData = solidJs.createMemo(() => props.buffer?.getChannelData(0));
  const duration = solidJs.createMemo(() => props.buffer?.duration ?? 0);
  const endTime = solidJs.createMemo(() => props.position + duration() / props.zoom);
  const dataLength = solidJs.createMemo(() => rawData()?.length ?? 0);
  const visibleLength = solidJs.createMemo(() => Math.min((endTime() - props.position) / duration() * dataLength(), dataLength()));
  const [canvasDimensions, setCanvasDimensions] = solidJs.createSignal({
    height: 0,
    width: 0,
    left: 0,
    top: 0
  });
  const cachedWaveformPeaks = solidJs.createMemo(() => createCachedWaveformSource(rawData() ?? []));
  ...
  return (() => {
    ...
    var _ref$ = canvasRef;
    typeof _ref$ === "function" ? web.use(_ref$, _el$2) : canvasRef = _el$2;
    ...
    typeof _ref$2 === "function" ? web.use(_ref$2, _el$5) : scrollbarDivRef = _el$5;
    web.insert(_el$7, web.createComponent(WaveformContextProvider, {
      value: contextValue,
      get children() {
...
var _tmpl$$2 = /*#__PURE__*/web.template(`<div><canvas style=position:absolute;left:0;top:0;width:100%;height:100%>`);
const Oscilloscope = allProps => {
  const propsWithDefaults = solidJs.mergeProps(allProps, {
    slowNessFactor: 250,
    scale: 1,
    mode: "peak",
    lineWidth: 1,
  ...
  const [props, divProps] = solidJs.splitProps(propsWithDefaults, ["analyzerNode", "slowNessFactor", "scale", "mode", "lineWidth", "strokeStyle"]);
  let animationFrame;
  let canvasRef;
  let context;
  let dimensions;
  const dataArray = solidJs.createMemo(() => new Float32Array(props.analyzerNode.frequencyBinCount));
  let lastDraw = 0;
  let peaks = [];
  const draw = () => {
...
var _tmpl$$1 = /*#__PURE__*/web.template(`<div class=Waveform-Regions style=position:absolute;left:0;top:0;width:100%;height:100%>`);
const Regions = props => {
  const [newRegion, setNewRegion] = solidJs.createSignal();
  const context = useWaveformContext();
  const viewPort = useWaveformViewPortScaler();
  ...
  return (() => {
    ...
    web.insert(_el$, web.createComponent(solidJs.Index, {
      ...
      children: region => web.createComponent(Region, {
        get region() {
          return region();
        },
        get duration() {
...
var _tmpl$ = /*#__PURE__*/web.template(`<div class=PlayHead>`);
const PlayHead = allProps => {
  const propsWithDefauls = solidJs.mergeProps({
    playHeadPosition: 0,
    sync: false
  }, allProps);
  const [props, divProps] = solidJs.splitProps(propsWithDefauls, ["playHeadPosition", "sync", "onPlayHeadPositionChange"]);
  const context = useWaveformContext();
  const viewPort = useWaveformViewPortScaler();
  solidJs.createEffect(() => {
  ...
  const leftPosition = solidJs.createMemo(() => viewPort.getCoordinates(props.playHeadPosition));
  return (() => {
    var _el$ = _tmpl$();
    web.spread(_el$, web.mergeProps(divProps, {
      get style() {
...
exports.createCachedWaveformPeaks = createCachedWaveformSource;
